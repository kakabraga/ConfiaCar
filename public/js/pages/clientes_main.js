...
async function refreshGrid() {
    try {
        ...
        if (Array.isArray(apiResponseData)) {
            ...
            console.log("2. Dados MAIPEADOS (array de arrays) para o formato do Grid.js para refresh:", mappedDataForGrid);

            // 3. Atualiza os dados da instância existente do Grid.js
            if (clientesGridInstance) {
                clientesGridInstance.updateConfig({
            ...
            console.error("Erro no refreshGrid: Resposta inesperada do servidor. Esperado um ARRAY de clientes.", apiResponseData);
            if (clientesGridInstance) {
...
document.addEventListener("DOMContentLoaded", async function () { // Adicionado 'async' aqui
    ...
    try {
        ...
        if (Array.isArray(apiResponseData)) {
            ...
            console.error("Erro na carga inicial de dados: Resposta inesperada do servidor.", apiResponseData);
        }
    } catch (error) {
        console.error("Erro ao carregar dados iniciais dos clientes:", error);
        alert("Não foi possível carregar os dados iniciais dos clientes. Verifique o console para detalhes.");
    }
